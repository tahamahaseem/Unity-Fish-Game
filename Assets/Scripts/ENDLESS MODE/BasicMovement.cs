...
public class BasicMovement : MonoBehaviour
{
    ...
    public static BasicMovement instance = null;
    ...
    if (Input.GetAxis("Horizontal") > 0){
     ...
     transform.position = transform.position + movement * (speed * Time.deltaTime);
        ...
        if ((Input.GetKeyDown("space") == true) && (boost == false) && (boostTime >= (boostDuration - 12)))
        {
            speed = speed * 5f;
            boost = true;
    ...
    public IEnumerator knockBackPlayer(float knockbackPower, Transform obj)
    {
            ...
            health.currentHealth--;
            rb2d.AddForce(direction * 200, ForceMode2D.Impulse);
